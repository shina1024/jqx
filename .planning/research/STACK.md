...
| TypeScript | 5.9.x | Public type surface for npm package and adapters | TypeScript 5.9 is the current stable line from official release notes and matches the repo's existing dependency direction |
...
| tsup | 8.5.1 | Build ESM, CJS, and `.d.ts` outputs for npm packages | Use for the public npm surfaces and adapter packages |
...
| `pnpm build`, `pnpm test`, `pnpm typecheck` | npm surface verification | Run after refreshing local `file:` dependencies in dependency order |
...
| `tsup` multi-entry packaging | Rollup or unbuild | Use if output customization becomes much more complex than the current runtime-plus-adapter package shape |
...
| Node 24 LTS | TypeScript 5.9.x, `tsup` 8.5.1, Node test runner | Stable npm packaging and test baseline |
...
- https://devblogs.microsoft.com/typescript/announcing-typescript-5-9/ - official TypeScript 5.9 release notes
