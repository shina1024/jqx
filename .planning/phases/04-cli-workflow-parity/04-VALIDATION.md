...
| **Config file** | `cmd/moon.pkg`, `.planning/REQUIREMENTS.md`, `scripts/jq_compat_cases.json`, `scripts/jq_exit_cases.json` |
| **Quick run command** | `moon test cmd` |
| **Full suite command** | `moon info && moon fmt && moon check && moon test && node scripts/jq_diff.mjs && node scripts/jq_diff.mjs scripts/jq_exit_cases.json` |
...
- **After every plan wave:** Run `moon info && moon fmt && moon check && moon test && node scripts/jq_diff.mjs && node scripts/jq_diff.mjs scripts/jq_exit_cases.json`
...
|---------|------|------|-------------|-----------|-------------------|-------------|--------|
| 04-01-01 | 01 | 1 | CLI-01 | jqx-first help and docs contract | `moon test cmd` | ✅ | ✅ green |
| 04-01-02 | 01 | 1 | CLI-01 | focused CLI workflow and output regressions | `moon test cmd` | ✅ | ✅ green |
| 04-02-01 | 02 | 2 | CLI-01, CLI-02 | maintained jq CLI corpus expansion | `node scripts/jq_diff.mjs && node scripts/jq_diff.mjs scripts/jq_exit_cases.json` | ✅ | ✅ green |
| 04-02-02 | 02 | 2 | CLI-01, CLI-02 | fast and native parity proof alignment | `moon test cmd && node scripts/jq_diff.mjs && node scripts/jq_diff.mjs scripts/jq_exit_cases.json` | ✅ | ✅ green |
...
- `scripts/jq_exit_cases.json` for dedicated `-e` exit-status parity cases
- `scripts/jq_diff.mjs` for native maintained differential proof

Wave 0 is complete for this phase because the existing wbtests and differential corpora already cover the CLI transport, option, module-path, raw-output, and exit-status proof expected by the final Phase 4 verification.
